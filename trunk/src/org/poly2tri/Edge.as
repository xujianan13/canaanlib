package org.poly2tri {
	...
	public class Edge {
		...
		public function classifyPoint(pt : Point, epsilon : Number = 0.000001) : int {
			var result : int = 0;
			var xa : Number = q.x - p.x;
			var ya : Number = q.y - p.y;

			var xb : Number = pt.x - p.x;
			var yb : Number = pt.y - p.y;

			var s : Number = xa * yb - xb * ya;
			if(s == 0) return 0;
			if(s > 0) return 1;
			if(s < 0) return -1;
		...
		public static function classifyPoint(pt : Point, p : Point, q : Point, epsilon : Number = 0.000001) : int {
			var result : int = 0;
			var xa : Number = q.x - p.x;
			var ya : Number = q.y - p.y;
